...
class TestParser(unittest.TestCase):
    def test_parser(self):
        ...
        if all:
            exprs["(R * temperature / (zCa * F)) * log(caConcExt / caConc)"] = (
                "(* (/ (* {R} {temperature}) (* {zCa} {F})) (log (/ {caConcExt} {caConc})))"
            )
            exprs["( (V - ((V^3) / 3)) - W + I) / SEC"] = (
                "(/ (+ (- (- {V} (/ (^ {V} {3.0}) {3.0})) {W}) {I}) {SEC})"
            )
            exprs["(V - (V^3) / 3 - W + I) / SEC"] = (
                "(/ (- {V} (+ (- (/ (^ {V} {3.0}) {3.0}) {W}) {I})) {SEC})"
            ...
            exprs["(120 + 300/( (exp ((V + 55)/9)) + (exp ((V + 65)/(-16)))))"] = (
                "(+ {120.0} (/ {300.0} (+ (exp (/ (+ {V} {55.0}) {9.0})) (exp (/ (+ {V} {65.0}) (- {0.0} {16.0}))))))"
            )

            exprs["(43.4 - 42.6/(1.0 + (exp ((V + 68.1)/(-20.5)))))"] = (
                "(- {43.4} (/ {42.6} (+ {1.0} (exp (/ (+ {V} {68.1}) (- {0.0} {20.5}))))))"
            )

            exprs["(2.64 - 2.52/(1.0 + (exp ((V+120)/(-25)))))"] = (
