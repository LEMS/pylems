...
def process_args():
    ...
    parser.add_argument(
        "lems_file", type=str, metavar="<LEMS file>", help="LEMS file to be simulated"
    )
        ...
        if target_comp.type == "network":
            for child in target_comp.children:
                if child.type == "population":
                    ...
                    export_component(
                        model, comp, sim_comp, child.id, file_name=dlems_file_name
                    )
        else:
...
def process_simulation_output(sim, model, options):
    ...
    for c in simulation.children:
        ...
        if c.type == "OutputFile":
            ...
            for f in c.children:
                file_column_order[c.parameters["fileName"]].append(
                    f.parameters["quantity"]
                )
        ...
        if runnable.recorded_variables:
            for recording in runnable.recorded_variables:
                if isinstance(recording.data_output, DataDisplay):
                    ...
                    raise Exception(
                        "Invalid output type - " + str(type(recording.data_output))
                    )
    ...
    for file_out_name in file_column_order.keys():
        ...
        print(
            "Going to save {0}x{1} data points to file {2}".format(
                len(times), len(vals.keys()), file_out_name
            )
        )
